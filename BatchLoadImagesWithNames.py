...
class LoadImagesFromDirBatch:
    ...
    # 修改点：新增 file paths 与 file names 两个 STRING 输出，保留 count INT
    # 返回类型顺序： IMAGE, MASK, FILE PATHS (list of STRING), FILE NAMES (list of STRING), COUNT (INT)
    RETURN_TYPES = ("IMAGE", "MASK", "STRING", "STRING", "INT")
    RETURN_NAMES = ("IMAGE", "MASK", "FILE PATHS", "FILE NAMES", "COUNT")
    # 对于 IMAGE/MASK/FILE PATHS/FILE NAMES 都作为列表输出（当返回多张时），COUNT 单独为 INT
    ...
    def load_images(self, directory: str, image_load_cap: int = 0, start_index: int = 0, load_always=False, sort_method=None):
        ...
        for image_path in dir_files:
            ...
            file_names.append(_basename_no_ext(image_path))  # 记录不含扩展名的文件名
            image_count += 1

        # 现在统一返回：当只有一张图片时也仍返回列表形式的 file_paths / file_names，
        # 但是 IMAGE 和 MASK 单独返回对应的张量（或 batch tensor）
        if len(images) == 1:
            # images[0], masks[0] are tensors for the single item
            return (images[0], masks[0], file_paths, file_names, 1)
            ...
            return (image1, mask1, file_paths, file_names, len(images))
